 ...
 */
const weexConfig = webpackMerge(commonConfig[1], {watch: true})
...
module.exports = new Promise((resolve, reject) => {
  ...
  // config.dev.port
  // portfinder.getPort((err, port) => { if (err) {     reject(err) } else {
  // publish the new Port, necessary for e2e tests
  let port = config.dev.port;
  let ip = config.dev.host;
  process.env.PORT = port;
  config.dev.port = port;
  // config.calculatePort = port; add port to devServer config
  ...
  devWebpackConfig
    ...
    .push(new FriendlyErrorsPlugin({
      compilationSuccessInfo: {
        messages: [`Your application is running here: ${chalk.yellow(`http://${devWebpackConfig.devServer.host}:${port}`)}.`]
      },
